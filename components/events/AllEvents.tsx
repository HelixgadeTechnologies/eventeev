...
import CreateEventForm from "@/components/events/createEventForms";
import { useAppDispatch, useAppSelector } from "@/store/hooks";
import { RootState } from "@/store/store";
import { setShowForm } from "@/store/features/create-event/createEventSlice";

export default function EventsComponent() {
  const [activeTab, setActiveTab] = useState(1);
  const { showForm } = useAppSelector((state: RootState) => state.createEvent);
        ...
        <>
          <div className="h-[52px] w-full gap-6 flex justify-between items-center text-sm border-b border-[#E4E7EC] my-6 relative">
            ...
            <div className="w-[200px]">
              <Button content="Create your first event" onClick={handleToggleForm} />
            </div>
          </div>
