...
const TableList = ({ data, onSpeakerClick }: { data: SpeakerDataType[], onSpeakerClick: (speaker: SpeakerDataType) => void }) => {
  const columns: ColumnDef<SpeakerDataType>[] = [
    ...
    },
    {
      ...
      cell: ({ row }) => (
        <button 
          onClick={() => onSpeakerClick(row.original)}
          className="font-black text-[#1B1818] uppercase tracking-tight text-xs hover:text-[#EB5017] transition-colors text-left"
        >
          {row.getValue("name")}
        </button>
      ),
